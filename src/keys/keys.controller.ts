...
export class KeysController {
  ...
  async createApiKey(@Request() req, @Body() createApiKeyDto: CreateApiKeyDto) {
    ...
    const result = await this.keysService.createApiKey(
      req.user.id,
  ...
  async listKeys(@Request() req) {
    ...
    return this.keysService.findAllByUserId(req.user.id);
  }
  ...
  async createApiKeyForUser(@Request() req, @Body() body: { userId: string; type: 'backend' | 'public'; name?: string }) {
    ...
    const result = await this.keysService.createApiKey(body.userId, body.type, body.name);
    return {
      id: result.id,
      key: result.key,
      message: 'Store this key securely. It will not be shown again.',
  ...
  async deleteApiKey(@Request() req, @Param('id') id: string) {
    ...
    return this.keysService.deleteApiKey(id);
  }
}
